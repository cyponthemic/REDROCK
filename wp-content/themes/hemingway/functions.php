...
function hemingway_load_javascript_files() {
	...
	if ( !is_admin() )
		...
		wp_register_script( 'vhlib', get_template_directory_uri().'/js/vhlib.js', array('jquery'), '', true );
		
		wp_enqueue_script( 'skrollr',true );
		wp_enqueue_script( 'vhlib' );
...
function  my_custom_thumbnails(){
echo woocommerce_get_product_thumbnail('large');
}

add_action('woocommerce_before_shop_loop_item_title','my_custom_thumbnails', 10)
